...
(function ($) {
	...
	const $body_page = $('body');
	const $main_logo1 = $('#main-logo-1');
	const $hamburger = $('#hamburger-open-btn');
		$hamburger.on('click', function () {
			if ($main_wrap.hasClass('menu-open')) {
				$main_wrap.removeClass('menu-open-temp');
				$body_page.removeClass('no-scroll');
				
				$main_logo1.removeClass('no-show');
				$main_logo1.addClass('show');
				...
				$body_page.addClass('no-scroll');

				$main_logo1.removeClass('show');
				$main_logo1.addClass('no-show');
			}
		$hamburger.toggleClass('active');
	});


	// Función para cerrar el menú al hacer clic en un link y hacer scroll suave
	$('.px-offcanvas-2-area nav ul li a').on('click', function (e) {
		const href = $(this).attr('href');

		// Solo actuar si es un enlace interno (ancla)
		if (href.startsWith('#')) {
			e.preventDefault();

			// 1. Lógica de cierre idéntica a tu función original
			$main_wrap.removeClass('menu-open-temp');
			$body_page.removeClass('no-scroll');
			
			$main_logo1.removeClass('no-show').addClass('show');

			$main_wrap.addClass('menu-open-temp');
			$main_wrap.removeClass('menu-open');
			
			// 2. Resetear el botón hamburguesa (quitar clase active)
			$hamburger.removeClass('active');

			// 3. Limpiar la clase temporal después de la animación
			setTimeout(() => {
				$main_wrap.removeClass('menu-open-temp');
			}, 2000);

			// 4. Scroll suave hacia la sección destino
			const target = $(href);
			if (target.length) {
				$('html, body').stop().animate({
	...
	// Función para cerrar el menú al hacer clic en un link y hacer scroll suave
	$('.px-footer-widget-menu ul li a').on('click', function (e) {
		const href = $(this).attr('href');
